...
namespace GraphicsText
{
    ...
    class Program
    {
        ...
        public void Count(string func)
        {
            PointF[] points = new PointF[1000];
            for (int x = 0; Parser.process(func) <= 1000; x++) //Да, да, "как парсеру понять что нужный x - это мой x". Никак. Это был тест.
            {
                points[x] = new PointF(x, 1001 - (float)Parser.process(func));
            }
            Draw(points);
        }
        ...
        {
            Bitmap bitmap1 = new Bitmap(1001, 1001);
            Graphics image1 = Graphics.FromImage(bitmap1);
            image1.Clear(Color.White);
            Pen karandashik = new Pen(Color.Black, 1);

            image1.DrawLine(karandashik, 0, 501, 1001, 501);
            image1.DrawLine(karandashik, 501, 0, 501, 1001);

            image1.DrawLines(karandashik, points);
            bitmap1.Save("testImage.jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
        }
    }
}